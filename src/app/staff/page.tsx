...
export default function StaffDashboard() {
  ...
  const PRIORITY_BADGES: Record<string,string> = { urgent: 'bg-danger/10 text-danger border-danger/30', hoog: 'bg-warning/10 text-warning border-warning/30', normaal: 'bg-gray-50 text-gray-500 border-gray-200', laag: 'bg-ocean/10 text-ocean border-ocean/30' };
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {[
          ...
          <motion.div key={s.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 + i * 0.06 }}
            ...
            <div className="text-xs font-bold text-gray-500/60 uppercase tracking-wider mt-1">{s.label}</div>
          </motion.div>
        ))}
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="card-premium overflow-hidden">
        <div className="p-5 border-b border-gray-200 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="font-bold text-gray-900">Mijn openstaande taken</h2>
          </div>
        ...
        <div className="divide-y divide-gray-100">
          {tasks.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <p className="text-sm text-gray-500/70 font-bold">Geen openstaande taken</p>
              <p className="text-xs text-gray-500/50 mt-1">Goed bezig!</p>
            </div>
          ) : tasks.map((t, i) => (
            <motion.div key={t.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 + i * 0.04 }}
              className={`p-4 flex items-center gap-4 border-l-4 ${PRIORITY_COLORS[t.priority] || 'border-l-slate-200'} hover:bg-gray-50 transition-colors`}>
              {t.priority === 'urgent' ? <AlertTriangle size={16} className="text-danger shrink-0" /> : <Clock size={16} className="text-gray-500/50 shrink-0" />}
              <div className="flex-1 min-w-0">
                <h3 className="text-sm font-bold text-gray-900">{t.title}</h3>
                <p className="text-xs text-gray-500/70 mt-0.5">{t.location_name || 'Alle locaties'} {t.due_date ? `· Deadline: ${new Date(t.due_date).toLocaleDateString('nl-NL')}` : ''}</p>
              </div>
              <span className={`text-xs font-bold px-2.5 py-1 rounded-full border ${PRIORITY_BADGES[t.priority] || 'bg-gray-50 text-gray-500 border-gray-200'}`}>{t.priority}</span>
            </motion.div>
          ))}
        </div>
      </motion.div>
