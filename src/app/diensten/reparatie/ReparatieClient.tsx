...
export default function ReparatieClient({ hourlyRate }: { hourlyRate: number }) {
  const { t } = useLocale();
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'var(--bg)' }}>
      ...
      <main id="main" className="flex-1">
        ...
        <Process t={t} />
        <FormSection />
        <UrgentBand t={t} hourlyRate={hourlyRate} />
      </main>
      <PublicFooter />
    </div>
  );
...
function UrgentBand({ t, hourlyRate }: { t: T; hourlyRate: number }) {
  const subText = hourlyRate > 0
    ? `${t('rep1.urg-sub')} Tarief vanaf ${fmtEur(hourlyRate)}/u of een vaste offerte-prijs.`
    : t('rep1.urg-sub');
  return (
      ...
      <div className="max-w-[1200px] mx-auto px-5 sm:px-10">
        <div className="cta-band-mk" style={{ background: 'var(--orange)', color: 'var(--navy)' }}>
          <div>
            ...
            <p style={{ marginTop: 8, color: 'rgba(31,42,54,0.78)', fontSize: 15, lineHeight: 1.55 }}>{subText}</p>
          </div>
