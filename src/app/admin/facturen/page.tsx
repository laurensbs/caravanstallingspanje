  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div><h1 className="text-2xl font-bold text-gray-900">Facturen</h1><p className="text-sm text-gray-500/70 mt-1">{total} facturen · Openstaand: {fmt(totalOpen)}</p></div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={openForm} className="bg-primary hover:bg-primary-light text-white font-bold px-5 py-2.5 rounded-xl text-sm flex items-center gap-2 shadow-lg shadow-primary/20 transition-all"><Plus size={16} /> Nieuwe factuur</button>
        </div>
      </div>
      ...
      <div className="bg-surface rounded-2xl border border-gray-200 overflow-hidden">
        ...
        <table className="w-full text-sm min-w-[800px]">
          <thead className="bg-gray-50 border-b border-gray-200"><tr>
            ...
            <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500/70 uppercase tracking-wider">Factuur</th>
            ...
            <th className="text-right px-4 py-3.5 text-xs font-semibold text-gray-500/70 uppercase tracking-wider">Bedrag</th>
          ...
          <tbody className="divide-y divide-gray-100">
            ...
            invoices.length === 0 ? <tr><td colSpan={8} className="px-4 py-8 text-center text-gray-500/70">Geen facturen gevonden</td></tr> :
            invoices.map(i => {
              ...
              return (
                <tr key={i.id} className={`hover:bg-gray-50 transition-colors ${selected.has(i.id) ? 'bg-primary/[0.03]' : ''}`}>
                  <td className="w-10 px-4 py-3">
                    <button onClick={() => toggleSelect(i.id)} className="text-gray-500/50 hover:text-gray-500 transition-colors" aria-label={`Selecteer ${i.invoice_number}`}>
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title="Nieuwe factuur">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="text-xs font-semibold text-gray-500 block mb-1">Contract (optioneel)</label>
            <select value={form.contract_id} onChange={e => { const ct = contracts.find(c => String(c.id) === e.target.value); setForm({...form, contract_id: e.target.value, subtotal: ct ? String(ct.monthly_rate) : form.subtotal, description: ct ? `Stalling ${ct.contract_number}` : form.description }); }} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-gray-50 focus:ring-2 focus:ring-primary/20 outline-none transition-all">
