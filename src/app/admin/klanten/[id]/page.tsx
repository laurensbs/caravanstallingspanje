...
export default function CustomerDetailPage() {
  ...
  if (!customer) return (
    <div className="text-center py-20">
      <p className="text-warm-gray/70 text-lg mb-4">Klant niet gevonden</p>
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        <Link href="/admin/klanten" className="p-2 hover:bg-sand/40 rounded-xl transition-colors text-warm-gray/70 hover:text-warm-gray">
          <ArrowLeft size={18} />
        </Link>
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setShowEdit(true)} className="flex items-center gap-2 px-4 py-2.5 bg-primary hover:bg-primary-dark text-white rounded-xl text-sm font-semibold shadow-lg shadow-primary/20 transition-all">
            <Edit2 size={14} /> Bewerken
          </button>
          <button onClick={() => setShowDelete(true)} className="p-2.5 hover:bg-danger/10 text-warm-gray/50 hover:text-danger rounded-xl transition-all" title="Verwijderen">
            <Trash2 size={16} />
          </button>
        </div>
      </div>
      ...
      {warnings.length > 0 && (
        <div className="bg-warning/10 border border-warning/30 rounded-2xl p-4 mb-6">
          ...
          <ul className="space-y-1">
            {warnings.map((w, i) => <li key={i} className="text-sm text-warm-gray">{w}</li>)}
          </ul>
        </div>
      )}

      {/* Tabs */}
      <div className="flex gap-1 bg-surface rounded-2xl p-1.5 mb-6 border border-sand-dark/20 overflow-x-auto">
        {tabs.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)} className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium transition-all whitespace-nowrap ${tab === t.key ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'text-warm-gray/70 hover:text-warm-gray hover:bg-sand/40'}`}>
            <t.icon size={15} />
            {t.label}
            {t.count !== undefined && t.count > 0 && <span className={`text-xs px-1.5 py-0.5 rounded-full font-bold ${tab === t.key ? 'bg-white/20' : 'bg-sand text-warm-gray/70'}`}>{t.count}</span>}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
            {[
              ...
              <div key={s.label} className="bg-surface rounded-2xl p-4 border border-sand-dark/20">
                <p className={`text-2xl font-black ${s.color}`}>{s.value}</p>
                <p className="text-xs text-warm-gray/70 mt-1">{s.label}</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-6">
            <div className="bg-surface rounded-2xl p-6 border border-sand-dark/20">
              <h3 className="font-bold text-surface-dark mb-4">Contactgegevens</h3>
              <div className="space-y-3">
                <div className="flex items-center gap-3 text-sm"><Mail size={15} className="text-warm-gray/50 shrink-0" /><span className="text-warm-gray">{customer.email}</span></div>
                {customer.phone && <div className="flex items-center gap-3 text-sm"><Phone size={15} className="text-warm-gray/50 shrink-0" /><span className="text-warm-gray">{customer.phone}</span></div>}
                {(customer.address || customer.city) && <div className="flex items-start gap-3 text-sm"><MapPin size={15} className="text-warm-gray/50 shrink-0 mt-0.5" /><span className="text-warm-gray">{[customer.address, customer.postal_code, customer.city, customer.country].filter(Boolean).join(', ')}</span></div>}
                {customer.company_name && <div className="flex items-center gap-3 text-sm"><Building2 size={15} className="text-warm-gray/50 shrink-0" /><span className="text-warm-gray">{customer.company_name}</span></div>}
                <div className="flex items-center gap-3 text-sm"><Calendar size={15} className="text-warm-gray/50 shrink-0" /><span className="text-warm-gray">Klant sinds {fmtDate(customer.created_at)}</span></div>
              </div>
            </div>
            <div className="bg-surface rounded-2xl p-6 border border-sand-dark/20">
              <h3 className="font-bold text-surface-dark mb-4">Notities</h3>
              <p className="text-sm text-warm-gray whitespace-pre-wrap">{customer.notes || 'Geen notities'}</p>
            </div>
          </div>
          ...
          {caravans.length > 0 && (
            <div className="bg-surface rounded-2xl p-6 border border-sand-dark/20">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-surface-dark">Caravans</h3>
              ...
              <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
                {caravans.slice(0, 3).map(c => (
                  <div key={c.id} className="bg-sand/40 rounded-xl p-4 border border-sand-dark/10">
                    <div className="flex items-center justify-between mb-2">
                      <span className="font-semibold text-surface-dark text-sm">{c.brand} {c.model || ''}</span>
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${CARAVAN_STATUS_COLORS[c.status] || 'bg-sand text-warm-gray'}`}>{c.status}</span>
                    </div>
                    <p className="text-xs text-warm-gray/70">{c.license_plate} · {c.location_name || 'Geen locatie'}{c.spot_label ? ` — ${c.spot_label}` : ''}</p>
                  </div>
                ))}
              </div>
            </div>
          ...
          {openServices > 0 && (
            <div className="bg-surface rounded-2xl p-6 border border-sand-dark/20">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-surface-dark">Open dienstaanvragen</h3>
              ...
              <div className="space-y-2">
                {serviceRequests.filter(s => s.status !== 'afgerond' && s.status !== 'geannuleerd').slice(0, 3).map(s => (
                  <div key={s.id} className="flex items-center justify-between bg-sand/40 rounded-xl p-3 border border-sand-dark/10">
                    <div>
                      <span className="font-semibold text-surface-dark text-sm">{s.service_type}</span>
                      <span className="text-xs text-warm-gray/70 ml-2">{fmtDate(s.created_at)}</span>
                    </div>
                    <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${SERVICE_STATUS_COLORS[s.status] || 'bg-sand text-warm-gray'}`}>{s.status}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <table className="w-full text-sm">
              <thead className="bg-sand/40 border-b border-sand-dark/20">
                <tr>
                  ...
                  <th className="text-left px-4 py-3.5 text-xs font-semibold text-warm-gray/70 uppercase tracking-wider">Kenteken</th>
                  <th className="text-left px-4 py-3.5 text-xs font-semibold text-warm-gray/70 uppercase tracking-wider">Locatie</th>
                  <th className="text-left px-4 py-3.5 text-xs font-semibold text-warm-gray/70 uppercase tracking-wider">Verzekering</th>
                  <th className="text-left px-4 py-3.5 text-xs font-semibold text-warm-gray/70 uppercase tracking-wider">APK</th>
                  <th className="text-left px-4 py-3.5 text-xs font-semibold text-warm-gray/70 uppercase tracking-wider">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-sand-dark/10">
                {caravans.length === 0 ? (
                  <tr><td colSpan={6} className="px-4 py-8 text-center text-warm-gray/70">Geen caravans</td></tr>
                ) : caravans.map(c => {
                  ...
                  return (
                    <tr key={c.id} className="hover:bg-sand/30 transition-colors">
                      <td className="px-4 py-3.5">
                        <p className="font-semibold text-surface-dark">{c.brand} {c.model || ''}</p>
                        {c.year && <p className="text-xs text-warm-gray/70">Bouwjaar {c.year}</p>}
                      </td>
                      <td className="px-4 py-3.5 font-mono text-xs">{c.license_plate || '-'}</td>
                      <td className="px-4 py-3.5 text-warm-gray">{c.location_name || '-'}{c.spot_label ? ` — ${c.spot_label}` : ''}</td>
                      <td className="px-4 py-3.5">
                        <span className={insExpiring ? 'text-danger font-semibold' : 'text-warm-gray'}>{fmtDate(c.insurance_expiry)}</span>
                        {insExpiring && <AlertTriangle size={12} className="inline ml-1 text-danger" />}
                      </td>
                      <td className="px-4 py-3.5">
                        <span className={apkExpiring ? 'text-danger font-semibold' : 'text-warm-gray'}>{fmtDate(c.apk_expiry)}</span>
        ...
        <div className="bg-surface rounded-2xl border border-sand-dark/20 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-sand/40 border-b border-sand-dark/20">
                <tr>
                  <th className="text-left px-4 py-3.5 text-xs font-semibold text-warm-gray/70 uppercase tracking-wider">Contractnr</th>
                  <th className="text-left px-4 py-3.5 text-xs font-semibold text-warm-gray/70 uppercase tracking-wider">Caravan</th>
                  <th className="text-left px-4 py-3.5 text-xs font-semibold text-warm-gray/70 uppercase tracking-wider">Periode</th>
                  <th className="text-left px-4 py-3.5 text-xs font-semibold text-warm-gray/70 uppercase tracking-wider">Maandtarief</th>
                  <th className="text-left px-4 py-3.5 text-xs font-semibold text-warm-gray/70 uppercase tracking-wider">Auto-verlengen</th>
                  <th className="text-left px-4 py-3.5 text-xs font-semibold text-warm-gray/70 uppercase tracking-wider">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-sand-dark/10">
                {contracts.length === 0 ? (
                  <tr><td colSpan={6} className="px-4 py-8 text-center text-warm-gray/70">Geen contracten</td></tr>
                ) : contracts.map(c => (
                  <tr key={c.id} className="hover:bg-sand/30 transition-colors">
                    <td className="px-4 py-3.5 font-mono text-xs">{c.contract_number}</td>
                    <td className="px-4 py-3.5 text-warm-gray">{c.caravan_name}</td>
                    <td className="px-4 py-3.5 text-warm-gray">{fmtDate(c.start_date)} — {fmtDate(c.end_date)}</td>
                    <td className="px-4 py-3.5 font-semibold text-surface-dark">{fmt(c.monthly_rate)}</td>
                    <td className="px-4 py-3.5 text-warm-gray">{c.auto_renew ? 'Ja' : 'Nee'}</td>
                    <td className="px-4 py-3.5">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${CONTRACT_STATUS_COLORS[c.status] || 'bg-sand text-warm-gray'}`}>{c.status}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {tab === 'facturen' && (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-surface rounded-2xl p-4 border border-sand-dark/20">
              <p className="text-xs text-warm-gray/70">Totaal betaald</p>
              <p className="text-xl font-black text-accent-dark">{fmt(totalRevenue)}</p>
            </div>
            <div className="bg-surface rounded-2xl p-4 border border-sand-dark/20">
              <p className="text-xs text-warm-gray/70">Openstaand</p>
              <p className="text-xl font-black text-warning">{fmt(invoices.filter(i => i.status === 'open' || i.status === 'verzonden').reduce((s, i) => s + Number(i.total), 0))}</p>
            </div>
            <div className="bg-surface rounded-2xl p-4 border border-sand-dark/20">
              <p className="text-xs text-warm-gray/70">Achterstallig</p>
              <p className="text-xl font-black text-danger">{fmt(invoices.filter(i => i.status !== 'betaald' && i.status !== 'geannuleerd' && i.due_date && new Date(i.due_date) < new Date()).reduce((s, i) => s + Number(i.total), 0))}</p>
            </div>
          </div>
          <div className="bg-surface rounded-2xl border border-sand-dark/20 overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                <thead className="bg-sand/40 border-b border-sand-dark/20">
                  <tr>
                    <th className="text-left px-4 py-3.5 text-xs font-semibold text-warm-gray/70 uppercase tracking-wider">Factuurnr</th>
                    <th className="text-left px-4 py-3.5 text-xs font-semibold text-warm-gray/70 uppercase tracking-wider">Omschrijving</th>
                    <th className="text-left px-4 py-3.5 text-xs font-semibold text-warm-gray/70 uppercase tracking-wider">Bedrag</th>
                ...
                <tbody className="divide-y divide-sand-dark/10">
                  {invoices.length === 0 ? (
                    <tr><td colSpan={5} className="px-4 py-8 text-center text-warm-gray/70">Geen facturen</td></tr>
                  ) : invoices.map(i => (
                    <tr key={i.id} className="hover:bg-sand/30 transition-colors">
                      <td className="px-4 py-3.5 font-mono text-xs">{i.invoice_number}</td>
                      <td className="px-4 py-3.5 text-warm-gray max-w-xs truncate">{i.description || '-'}</td>
                      <td className="px-4 py-3.5 font-semibold text-surface-dark">{fmt(i.total)}</td>
                      <td className="px-4 py-3.5 text-warm-gray">{fmtDate(i.due_date)}</td>
                      <td className="px-4 py-3.5">
                        <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${INVOICE_STATUS_COLORS[i.status] || 'bg-sand text-warm-gray'}`}>{i.status}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="bg-surface rounded-2xl border border-sand-dark/20 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-sand/40 border-b border-sand-dark/20">
                <tr>
                  <th className="text-left px-4 py-3.5 text-xs font-semibold text-warm-gray/70 uppercase tracking-wider">Type</th>
                  <th className="text-left px-4 py-3.5 text-xs font-semibold text-warm-gray/70 uppercase tracking-wider">Caravan</th>
                  <th className="text-left px-4 py-3.5 text-xs font-semibold text-warm-gray/70 uppercase tracking-wider">Omschrijving</th>
              ...
              <tbody className="divide-y divide-sand-dark/10">
                {serviceRequests.length === 0 ? (
                  <tr><td colSpan={5} className="px-4 py-8 text-center text-warm-gray/70">Geen dienstaanvragen</td></tr>
                ) : serviceRequests.map(s => (
                  <tr key={s.id} className="hover:bg-sand/30 transition-colors">
                    <td className="px-4 py-3.5 font-semibold text-surface-dark">{s.service_type}</td>
                    <td className="px-4 py-3.5 text-warm-gray">{s.caravan_name || '-'}</td>
                    <td className="px-4 py-3.5 text-warm-gray max-w-xs truncate">{s.description || '-'}</td>
                    <td className="px-4 py-3.5 text-warm-gray">{fmtDate(s.created_at)}</td>
                    <td className="px-4 py-3.5">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${SERVICE_STATUS_COLORS[s.status] || 'bg-sand text-warm-gray'}`}>{s.status}</span>
                    </td>
                  </tr>
      ...
      {tab === 'berichten' && (
        <div className="bg-surface rounded-2xl border border-sand-dark/20 overflow-hidden">
          {messages.length === 0 ? (
            <div className="p-8 text-center text-warm-gray/70">Geen berichten gevonden</div>
          ) : (
            <div className="divide-y divide-sand-dark/10">
              {messages.map(m => (
                <div key={m.id} className={`p-4 hover:bg-sand/30 transition-colors ${!m.is_read ? 'bg-primary/[0.03]' : ''}`}>
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      {!m.is_read && <div className="w-2 h-2 bg-primary rounded-full" />}
                      <span className="font-semibold text-surface-dark text-sm">{m.subject || 'Geen onderwerp'}</span>
                    </div>
                    <span className="text-xs text-warm-gray/50 flex items-center gap-1"><Clock size={11} /> {fmtDate(m.created_at)}</span>
                  </div>
                  <p className="text-sm text-warm-gray line-clamp-2">{m.message}</p>
                </div>
              ))}
            </div>
          )}
      ...
      <Modal open={showEdit} onClose={() => setShowEdit(false)} title="Klant bewerken" size="lg">
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <div className="flex gap-3 justify-end pt-2">
            <button type="button" onClick={() => setShowEdit(false)} className="px-4 py-2.5 text-sm font-medium text-warm-gray/70 hover:text-warm-gray hover:bg-sand/40 rounded-xl transition-all">Annuleren</button>
            <button type="submit" disabled={saving} className="bg-primary hover:bg-primary-dark text-white font-bold px-6 py-2.5 rounded-xl text-sm shadow-lg shadow-primary/20 transition-all disabled:opacity-60">{saving ? 'Opslaan...' : 'Opslaan'}</button>
          </div>
        </form>
      </Modal>
      ...
      <Modal open={showDelete} onClose={() => setShowDelete(false)} title="Klant verwijderen" size="sm">
        <p className="text-sm text-warm-gray mb-6">Weet u zeker dat u <strong>{customer.first_name} {customer.last_name}</strong> wilt verwijderen? Dit kan niet ongedaan worden gemaakt.</p>
        <div className="flex gap-3 justify-end">
          <button onClick={() => setShowDelete(false)} className="px-4 py-2.5 text-sm font-medium text-warm-gray/70 hover:text-warm-gray hover:bg-sand/40 rounded-xl transition-all">Annuleren</button>
