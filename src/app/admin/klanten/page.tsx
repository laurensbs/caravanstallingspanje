    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-500/70 mt-1">{total.toLocaleString('nl-NL')} klanten totaal</p>
        </div>
      ...
      <div className="bg-surface rounded-2xl border border-gray-200 mb-6 p-4">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 bg-gray-50 rounded-xl px-3.5 py-2.5 border border-gray-200 flex-1 min-w-[200px]">
            ...
            <input placeholder="Zoek op naam, email, klantnummer..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="bg-transparent text-sm outline-none flex-1 text-gray-500 placeholder:text-gray-500/50" />
        ...
        {selected.size > 0 && (
          <div className="mt-3 flex items-center gap-3 bg-primary/[0.06] rounded-xl px-4 py-2.5 border border-primary/20">
            ...
            <button onClick={() => setSelected(new Set())} className="text-xs text-gray-500/70 hover:text-gray-500 underline">Deselecteren</button>
          </div>
        )}
      </div>
      ...
      <div className="bg-surface rounded-2xl border border-gray-200 overflow-hidden">
            ...
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="w-10 px-4 py-3.5">
                  <button onClick={toggleAll} className="text-gray-500/50 hover:text-gray-500 transition-colors" aria-label="Alles selecteren">
                    {selected.size === customers.length && customers.length > 0 ? <CheckSquare size={16} className="text-primary" /> : <Square size={16} />}
                  </button>
                </th>
                <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500/70 uppercase tracking-wider">Klantnr</th>
                <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500/70 uppercase tracking-wider">Naam</th>
                <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500/70 uppercase tracking-wider">Contact</th>
                <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500/70 uppercase tracking-wider">Plaats</th>
                <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500/70 uppercase tracking-wider">Land</th>
                <th className="text-right px-4 py-3.5 text-xs font-semibold text-gray-500/70 uppercase tracking-wider">Acties</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500/70">Laden...</td></tr>
              ) : customers.length === 0 ? (
                <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500/70">Geen klanten gevonden</td></tr>
              ) : customers.map(c => (
                <tr key={c.id} className={`hover:bg-gray-50 transition-colors ${selected.has(c.id) ? 'bg-primary/[0.03]' : ''}`}>
                  <td className="w-10 px-4 py-3.5">
                    <button onClick={() => toggleSelect(c.id)} className="text-gray-500/50 hover:text-gray-500 transition-colors" aria-label={`Selecteer ${c.first_name} ${c.last_name}`}>
                  ...
                  <td className="px-4 py-3.5 text-right">
                    <div className="flex items-center justify-end gap-1">
                      <button onClick={() => openEdit(c)} className="p-1.5 hover:bg-gray-300/20 rounded-lg text-gray-500/70 hover:text-gray-500 transition-colors" title="Bewerken"><Edit2 size={14} /></button>
                      <a href={`/admin/klanten/${c.id}`} className="p-1.5 hover:bg-gray-300/20 rounded-lg text-gray-500/70 hover:text-gray-500 transition-colors" title="Details"><Eye size={14} /></a>
                    </div>
                  </td>
                </tr>
              ))}
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-200">
            <p className="text-xs text-gray-500/70">Pagina {page} van {totalPages}</p>
            <div className="flex gap-1">
              <button disabled={page <= 1} onClick={() => setPage(page - 1)} className="p-1.5 rounded-lg hover:bg-gray-300/20 disabled:opacity-30 text-gray-500/70 transition-colors" aria-label="Vorige pagina"><ChevronLeft size={16} /></button>
              <button disabled={page >= totalPages} onClick={() => setPage(page + 1)} className="p-1.5 rounded-lg hover:bg-gray-300/20 disabled:opacity-30 text-gray-500/70 transition-colors" aria-label="Volgende pagina"><ChevronRight size={16} /></button>
            </div>
          </div>
        )}
      </div>
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title={editing ? 'Klant bewerken' : 'Nieuwe klant'}>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-3 justify-end pt-2">
                <button type="button" onClick={() => setShowForm(false)} className="px-4 py-2.5 text-sm font-medium text-gray-500/70 hover:text-gray-500 hover:bg-gray-50 rounded-xl transition-all">Annuleren</button>
                <button type="submit" className="bg-primary hover:bg-primary-light text-white font-bold px-6 py-2.5 rounded-xl text-sm shadow-lg shadow-primary/20 transition-all">Opslaan</button>
              </div>
            </form>
      </Modal>
    </div>
