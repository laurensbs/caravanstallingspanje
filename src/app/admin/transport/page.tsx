  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div><h1 className="text-2xl font-bold text-gray-900">{t('Transport')}</h1><p className="text-sm text-gray-500/70 mt-1">{total} {t('transportopdrachten')}</p></div>
        <button onClick={openForm} className="bg-primary hover:bg-primary-light text-white font-bold px-5 py-2.5 rounded-xl text-sm flex items-center gap-2 shadow-lg shadow-primary/20 transition-all"><Plus size={16} /> {t('Nieuwe opdracht')}</button>
      </div>
      ...
      <div className="bg-surface rounded-2xl border border-gray-200 mb-6 p-4">
        <div className="flex gap-2 flex-wrap">
          {['', 'aangevraagd', 'gepland', 'onderweg', 'afgeleverd', 'geannuleerd'].map(s => (
            <button key={s} onClick={() => { setStatusFilter(s); setPage(1); }} className={`px-3.5 py-2 rounded-xl text-sm font-semibold transition-all ${statusFilter === s ? 'bg-primary text-white shadow-md shadow-primary/20' : 'bg-gray-50 hover:bg-gray-300/20 text-gray-500'}`}>{s || t('Alle')}</button>
          ))}
        </div>
      </div>
      ...
      <div className="bg-surface rounded-2xl border border-gray-200 overflow-hidden">
        ...
        <table className="w-full text-sm min-w-[800px]">
          <thead className="bg-gray-50 border-b border-gray-200"><tr>
            ...
            <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500/70 uppercase tracking-wider">{t('Route')}</th>
            ...
            <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500/70 uppercase tracking-wider">{t('Chauffeur')}</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500/70">{t('Laden...')}</td></tr> :
            orders.length === 0 ? <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500/70">{t('Geen transportopdrachten')}</td></tr> :
            orders.map(o => (
              <tr key={o.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-4 py-3"><div className="font-medium text-gray-900">{o.caravan_brand} {o.caravan_model}</div><div className="text-xs text-gray-500/70">{o.caravan_license_plate}</div></td>
                <td className="px-4 py-3"><div className="flex items-center gap-1 text-xs text-gray-500"><MapPin size={12} className="text-gray-500/50 flex-shrink-0"/><span className="truncate max-w-[100px]">{o.pickup_address || t('Stalling')}</span><ArrowRight size={12} className="text-gray-500/50 flex-shrink-0"/><span className="truncate max-w-[100px]">{o.delivery_address || t('Stalling')}</span></div></td>
                <td className="px-4 py-3 text-xs text-gray-500">{o.customer_name}</td>
                <td className="px-4 py-3 text-xs text-gray-500">{fmtDate(o.scheduled_date)}</td>
                <td className="px-4 py-3 text-xs text-gray-500">{o.assigned_staff_name || t('Niet toegewezen')}</td>
                <td className="px-4 py-3 text-center"><span className={`text-xs font-medium px-2 py-1 rounded-full ${TRANSPORT_STATUS_COLORS[o.status] || 'bg-gray-100'}`}>{o.status}</span></td>
                <td className="px-4 py-3 text-right">
                  <select value={o.status} onChange={e => updateStatus(o.id, e.target.value)} className="text-xs border border-gray-200 rounded-lg px-2 py-1 bg-gray-50 focus:ring-2 focus:ring-primary/20 outline-none">
                    <option value="aangevraagd">{t('Aangevraagd')}</option>
                    <option value="gepland">{t('Gepland')}</option>
                    <option value="onderweg">{t('Onderweg')}</option>
                    <option value="afgeleverd">{t('Afgeleverd')}</option>
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title={t('Nieuwe transportopdracht')}>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="text-xs font-semibold text-gray-500 block mb-1">{t('Chauffeur (optioneel)')}</label>
                <select value={form.assigned_staff_id} onChange={e=>setForm({...form,assigned_staff_id:e.target.value})} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-gray-50 focus:ring-2 focus:ring-primary/20 outline-none transition-all">
                  <option value="">{t('Niet toegewezen')}</option>
                  {staffList.map(s => <option key={s.id} value={s.id}>{s.first_name} {s.last_name}</option>)}
                </select>
              </div>
              <div><label className="text-xs font-semibold text-gray-500 block mb-1">{t('Ophaaladres')}</label><input value={form.pickup_address} onChange={e=>setForm({...form,pickup_address:e.target.value})} placeholder={t('Bijv. Stalling Costa del Sol')} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-gray-50 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all"/></div>
              <div><label className="text-xs font-semibold text-gray-500 block mb-1">{t('Afleveradres')}</label><input value={form.delivery_address} onChange={e=>setForm({...form,delivery_address:e.target.value})} placeholder={t('Bijv. Camping La Marina, Alicante')} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-gray-50 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all"/></div>
              <div><label className="text-xs font-semibold text-gray-500 block mb-1">{t('Geplande datum')} *</label><input type="date" required value={form.scheduled_date} onChange={e=>setForm({...form,scheduled_date:e.target.value})} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-gray-50 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all"/></div>
              <div><label className="text-xs font-semibold text-gray-500 block mb-1">{t('Opmerkingen')}</label><textarea value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} placeholder={t('Bijzonderheden voor transport...')} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-gray-50 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all" rows={3}/></div>
