  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div><h1 className="text-2xl font-bold text-gray-900">Transport</h1><p className="text-sm text-gray-500/70 mt-1">{total} transportopdrachten</p></div>
        <button onClick={openForm} className="bg-primary hover:bg-primary-light text-white font-bold px-5 py-2.5 rounded-xl text-sm flex items-center gap-2 shadow-lg shadow-primary/20 transition-all"><Plus size={16} /> Nieuwe opdracht</button>
      </div>
      ...
      <div className="bg-surface rounded-2xl border border-gray-200 mb-6 p-4">
        <div className="flex gap-2 flex-wrap">
          {['', 'aangevraagd', 'gepland', 'onderweg', 'afgeleverd', 'geannuleerd'].map(s => (
            <button key={s} onClick={() => { setStatusFilter(s); setPage(1); }} className={`px-3.5 py-2 rounded-xl text-sm font-semibold transition-all ${statusFilter === s ? 'bg-primary text-white shadow-md shadow-primary/20' : 'bg-gray-50 hover:bg-gray-300/20 text-gray-500'}`}>{s || 'Alle'}</button>
          ))}
        </div>
      </div>
      ...
      <div className="bg-surface rounded-2xl border border-gray-200 overflow-hidden">
        ...
        <table className="w-full text-sm min-w-[800px]">
          <thead className="bg-gray-50 border-b border-gray-200"><tr>
            ...
            <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500/70 uppercase tracking-wider">Route</th>
            ...
            <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500/70 uppercase tracking-wider">Chauffeur</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500/70">Laden...</td></tr> :
            orders.length === 0 ? <tr><td colSpan={7} className="px-4 py-8 text-center text-gray-500/70">Geen transportopdrachten</td></tr> :
            orders.map(o => (
              <tr key={o.id} className="hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3"><div className="flex items-center gap-1 text-xs text-gray-500"><MapPin size={12} className="text-gray-500/50 flex-shrink-0"/><span className="truncate max-w-[100px]">{o.pickup_address || 'Stalling'}</span><ArrowRight size={12} className="text-gray-500/50 flex-shrink-0"/><span className="truncate max-w-[100px]">{o.delivery_address || 'Stalling'}</span></div></td>
                ...
                <td className="px-4 py-3 text-xs text-gray-500">{o.assigned_staff_name || 'Niet toegewezen'}</td>
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title="Nieuwe transportopdracht">
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-gray-500 block mb-1">Caravan *</label>
              ...
              <div>
                <label className="text-xs font-semibold text-gray-500 block mb-1">Chauffeur (optioneel)</label>
              ...
              <div><label className="text-xs font-semibold text-gray-500 block mb-1">Ophaaladres</label><input value={form.pickup_address} onChange={e=>setForm({...form,pickup_address:e.target.value})} placeholder="Bijv. Stalling Costa del Sol" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-gray-50 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all"/></div>
              <div><label className="text-xs font-semibold text-gray-500 block mb-1">Afleveradres</label><input value={form.delivery_address} onChange={e=>setForm({...form,delivery_address:e.target.value})} placeholder="Bijv. Camping La Marina, Alicante" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-gray-50 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all"/></div>
              <div><label className="text-xs font-semibold text-gray-500 block mb-1">Geplande datum *</label><input type="date" required value={form.scheduled_date} onChange={e=>setForm({...form,scheduled_date:e.target.value})} className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-gray-50 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all"/></div>
              <div><label className="text-xs font-semibold text-gray-500 block mb-1">Opmerkingen</label><textarea value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} placeholder="Bijzonderheden voor transport..." className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm bg-gray-50 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all" rows={3}/></div>
