  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Locaties & Plekken</h1>
          <p className="text-sm text-gray-500/70 mt-1">Beheer uw terreinen en stallingplekken</p>
        </div>
                ...
                <div className="border-t border-gray-200 p-6">
                  <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-gray-900">Terreinkaart</span>
                  ...
                  {showBulk === loc.id && (
                    <div className="bg-gray-50 rounded-xl p-4 mb-4 flex flex-wrap gap-3">
                      <input placeholder="Zone" value={bulkForm.zone} onChange={e => setBulkForm({ ...bulkForm, zone: e.target.value })} className="border border-gray-200 rounded-lg px-3 py-2 text-sm w-20 bg-surface" />
                      <input placeholder="Prefix" value={bulkForm.prefix} onChange={e => setBulkForm({ ...bulkForm, prefix: e.target.value })} className="border border-gray-200 rounded-lg px-3 py-2 text-sm w-20 bg-surface" />
                      <input type="number" placeholder="Aantal" value={bulkForm.count} onChange={e => setBulkForm({ ...bulkForm, count: e.target.value })} className="border border-gray-200 rounded-lg px-3 py-2 text-sm w-24 bg-surface" />
                      <select value={bulkForm.spot_type} onChange={e => setBulkForm({ ...bulkForm, spot_type: e.target.value })} className="border border-gray-200 rounded-lg px-3 py-2 text-sm w-28 bg-surface"><option value="buiten">Buiten</option><option value="binnen">Binnen</option></select>
                      ...
                      <button onClick={() => setShowBulk(null)} className="text-gray-500/70 text-sm">Annuleren</button>
                    </div>
                  )}

                  {spotsLoading ? (
                    <p className="text-gray-500/70 text-sm">Laden...</p>
                  ) : spots.length === 0 ? (
                    <p className="text-gray-500/70 text-sm">Nog geen plekken aangemaakt. Gebruik de knop hierboven om plekken toe te voegen.</p>
                  ) : (
                    zones.map(zone => (
                      <div key={zone} className="mb-6">
                        <h4 className="text-sm font-semibold text-gray-500/70 mb-2">Zone {zone}</h4>
      ...
      <Modal open={showForm} onClose={() => setShowForm(false)} title="Nieuwe locatie" size="sm">
            <form onSubmit={handleAddLocation} className="space-y-4">
              ...
              <div className="flex gap-3 justify-end pt-2">
                ...
                <button type="submit" className="bg-primary hover:bg-primary-light text-white font-semibold px-6 py-2.5 rounded-xl text-sm shadow-lg shadow-primary/20 transition-all">Opslaan</button>
              </div>
            </form>
      </Modal>
    </div>
