...
function StatCard({ icon: Icon, label, value, sub, trend, color }: { icon: React.ElementType; label: string; value: string; sub?: string; trend?: number; color: string }) {
  return (
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="bg-surface rounded-2xl border border-sand-dark/20 p-6 hover:shadow-lg hover:shadow-sand-dark/20 transition-all">
      ...
      <p className="text-2xl font-black text-surface-dark">{value}</p>
      <p className="text-sm text-warm-gray/70 mt-1">{label}</p>
      {sub && <p className="text-xs text-warm-gray/50 mt-0.5">{sub}</p>}
    </motion.div>
  );
}
...
function OccupancyBar({ location, occupied, total }: { location: string; occupied: number; total: number }) {
  ...
  return (
    <div className="flex items-center gap-4">
      <div className="w-32 text-sm font-medium text-surface-dark truncate">{location}</div>
      ...
      <div className="w-16 text-right text-sm font-bold text-warm-gray">{pct}%</div>
      <div className="w-20 text-right text-xs text-warm-gray/70">{occupied}/{total}</div>
    </div>
  );
}
...
export default function RapportagesPage() {
  ...
  if (!stats) return (
    <div className="text-center py-32">
      <BarChart3 size={48} className="text-warm-gray/40 mx-auto mb-4" />
      <p className="text-warm-gray/70">Kon rapportages niet laden</p>
    </div>
  );
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-surface-dark">Rapportages & Analytics</h1>
          <p className="text-sm text-warm-gray/70 mt-1">Overzicht van bezetting, omzet en klantstatistieken</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex bg-sand rounded-lg p-0.5">
            {["maand", "kwartaal", "jaar"].map(p => (
              <button key={p} onClick={() => setPeriod(p)} className={`px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${period === p ? 'bg-surface text-surface-dark shadow-sm' : 'text-warm-gray/70 hover:text-warm-gray'}`}>
                {p.charAt(0).toUpperCase() + p.slice(1)}
              </button>
            ))}
          </div>
          <div className="relative">
            <button onClick={() => setShowExport(!showExport)} className="flex items-center gap-2 text-sm font-semibold text-warm-gray bg-surface border border-sand-dark/30 px-4 py-2 rounded-xl hover:bg-sand/40 transition-all">
              <Download size={14} /> Exporteren
            </button>
            {showExport && (
              <div className="absolute right-0 top-full mt-2 bg-surface rounded-xl border border-sand-dark/20 shadow-xl py-2 w-56 z-50">
                {[
                  ...
                  <a key={item.type} href={`/api/admin/export?type=${item.type}`} className="block px-4 py-2 text-sm text-warm-gray hover:bg-sand/40 hover:text-surface-dark transition-colors" onClick={() => setShowExport(false)}>
                    {item.label}
                  </a>
                ))}
              </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-surface rounded-2xl border border-sand-dark/20 p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="font-bold text-surface-dark">Omzet overzicht</h3>
              <p className="text-xs text-warm-gray/70 mt-0.5">Maandelijkse omzet in €</p>
            </div>
            <TrendingUp size={18} className="text-warm-gray/50" />
          </div>
          <BarChartSimple
        ...
        <div className="bg-surface rounded-2xl border border-sand-dark/20 p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="font-bold text-surface-dark">Bezetting per locatie</h3>
              <p className="text-xs text-warm-gray/70 mt-0.5">Plekken bezet / totaal</p>
            </div>
            <MapPin size={18} className="text-warm-gray/50" />
          </div>
          <div className="space-y-4">
            {stats.occupancyByLocation.length > 0 ? (
              ...
              <div className="text-center py-8">
                <MapPin size={32} className="text-warm-gray/40 mx-auto mb-2" />
                <p className="text-xs text-warm-gray/70">Geen locatiedata beschikbaar</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="bg-surface rounded-2xl border border-sand-dark/20 p-6">
          <h3 className="font-bold text-surface-dark mb-6">Contracten per type</h3>
            ...
            {(stats.contractsByType.length > 0 ? stats.contractsByType : [
              ...
              return (
                <div key={ct.type} className="flex items-center gap-4">
                  <div className="w-24 text-sm font-medium text-surface-dark">{ct.type}</div>
                  <div className="flex-1 h-2 bg-sand rounded-full overflow-hidden">
                    <div className="h-full bg-accent rounded-full" style={{ width: `${pct}%` }} />
                  </div>
                  <div className="w-8 text-right text-sm font-bold text-warm-gray">{ct.count}</div>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-surface rounded-2xl border border-sand-dark/20 p-6">
          <div className="flex items-center justify-between mb-6">
            <h3 className="font-bold text-surface-dark">Openstaande facturen</h3>
            <Receipt size={18} className="text-warm-gray/50" />
          </div>
          <div className="text-center py-6">
            <p className="text-4xl font-black text-warning">{stats.outstandingInvoices}</p>
            <p className="text-sm text-warm-gray/70 mt-2">openstaande facturen</p>
          </div>
          <a href="/admin/facturen" className="block text-center text-sm font-semibold text-accent hover:text-primary-dark transition-colors mt-4">
            Bekijk facturen →
          </a>
        </div>
      </div>
