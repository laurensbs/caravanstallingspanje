  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <div><h1 className="text-2xl font-bold text-gray-900">Contracten</h1><p className="text-sm text-gray-500/70 mt-1">{total} contracten · Maandomzet actief: {fmt(totalMonthly)}</p></div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={openForm} className="bg-primary hover:bg-primary-light text-white font-bold px-5 py-2.5 rounded-xl text-sm flex items-center gap-2 shadow-lg shadow-primary/20 transition-all"><Plus size={16} /> Nieuw contract</button>
        </div>
      </div>
      ...
      {selected.size > 0 && (
        <div className="mb-4 flex items-center gap-3 bg-primary/[0.06] rounded-xl px-4 py-3 border border-primary/20">
          ...
          <button onClick={() => setSelected(new Set())} className="text-xs text-gray-500/70 hover:text-gray-500 underline ml-auto">Deselecteren</button>
        </div>
      )}
      ...
      <div className="bg-surface rounded-2xl border border-gray-200 overflow-hidden">
        ...
        <table className="w-full text-sm min-w-[800px]">
          <thead className="bg-gray-50 border-b border-gray-200"><tr>
            ...
            <th className="text-left px-4 py-3.5 text-xs font-semibold text-gray-500/70 uppercase tracking-wider">Contract</th>
            ...
            <th className="text-right px-4 py-3.5 text-xs font-semibold text-gray-500/70 uppercase tracking-wider">Maandtarief</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {loading ? <tr><td colSpan={9} className="px-4 py-8 text-center text-gray-500/70">Laden...</td></tr> :
            contracts.length === 0 ? <tr><td colSpan={9} className="px-4 py-8 text-center text-gray-500/70">Geen contracten gevonden</td></tr> :
            contracts.map(c => (
              <tr key={c.id} className={`hover:bg-gray-50 transition-colors ${selected.has(c.id) ? 'bg-primary/[0.03]' : ''}`}>
                <td className="w-10 px-4 py-3">
                  <button onClick={() => toggleSelect(c.id)} className="text-gray-500/50 hover:text-gray-500 transition-colors" aria-label={`Selecteer ${c.contract_number}`}>
                ...
                <td className="px-4 py-3 text-right font-medium text-gray-900">{fmt(Number(c.monthly_rate))}/mnd</td>
                <td className="px-4 py-3 text-center">
                  ...
                  {c.auto_renew && <span title="Auto-verlenging"><RefreshCw size={12} className="inline ml-1 text-gray-500/70" /></span>}
                </td>
