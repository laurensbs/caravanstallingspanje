...
import { User, FileText, Receipt, Wrench, Truck, LogOut, Eye, EyeOff, Shield, Caravan, MapPin, Calendar, AlertCircle, CheckCircle2, Clock, Upload, CreditCard, Download, Bell, MessageSquare, Send, Activity, KeyRound, ClipboardCheck, Settings, Gift, Copy, CheckCircle } from 'lucide-react';
...
function MijnAccountContent() {
  ...
  const TABS = [
    ...
    { id: 'doorverwijzen', label: 'Vrienden uitnodigen', icon: Gift },
                  ...
                  {caravans.map(c => (
                    <div key={c.id} className="bg-white rounded-xl border border-sand-dark/10 p-4 flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <span className={`inline-flex items-center gap-1 text-[11px] font-bold px-2.5 py-1 rounded-full ${c.status === 'gestald' ? 'bg-accent/10 text-accent' : 'bg-warning/10 text-warning'}`}>
            ...
            <div className="bg-surface rounded-2xl border border-sand-dark/20 p-8">
              ...
              <div className="mb-6">
                ...
                <div className="flex items-center gap-2">
                  <input
                    readOnly
                    value={`https://caravanstalling-spanje.com/reserveren?ref=${customer?.customer_number || 'XXXXX'}`}
                    className="flex-1 border border-sand-dark/30 rounded-xl px-4 py-3 text-sm bg-sand/40 focus:outline-none"
                  />
                  <button
                    onClick={() => {
                      navigator.clipboard.writeText(`https://caravanstalling-spanje.com/reserveren?ref=${customer?.customer_number || ''}`);
                    }}
