  ...
  if (contracts.length === 0) {
    return (
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="card-premium p-12 text-center">
        ...
        <h3 className="font-bold text-gray-900 text-lg mb-1">Geen contracten</h3>
        <p className="text-sm text-gray-500/70">U heeft nog geen actieve contracten.</p>
      </motion.div>
    );
  }
          ...
          <motion.div key={c.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
            ...
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="font-bold text-gray-900">{c.contract_number}</h3>
                  <p className="text-xs text-gray-500/70">{fmt(c.monthly_rate)}/maand</p>
                </div>
              </div>
              <span className={`text-xs font-bold px-3.5 py-1.5 rounded-full border ${CUSTOMER_STATUS_COLORS[c.status] || 'bg-gray-50 text-gray-500 border-gray-200'}`}>{c.status}</span>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              <div className="bg-gray-50 rounded-xl p-3">
                <div className="flex items-center gap-1.5 mb-1">
                  <Calendar size={12} className="text-ocean/70" />
                  <span className="text-xs font-bold text-gray-500/60 uppercase tracking-wider">Periode</span>
                </div>
                <p className="text-sm font-medium text-gray-900">{fmtDate(c.start_date)} — {fmtDate(c.end_date)}</p>
              </div>
              <div className="bg-gray-50 rounded-xl p-3">
                <div className="flex items-center gap-1.5 mb-1">
                  <RefreshCw size={12} className="text-ocean/70" />
                  <span className="text-xs font-bold text-gray-500/60 uppercase tracking-wider">Auto-verlenging</span>
                </div>
                <div className="flex items-center gap-1.5">
                  {c.auto_renew ? <CheckCircle2 size={14} className="text-accent" /> : <Clock size={14} className="text-gray-500/50" />}
                  <span className="text-sm font-medium text-gray-900">{c.auto_renew ? 'Actief' : 'Uit'}</span>
                </div>
              </div>
              <div className="bg-gradient-to-br from-primary/[0.06] to-transparent rounded-xl p-3">
                <span className="text-xs font-bold text-gray-500/60 uppercase tracking-wider block mb-1">Maandbedrag</span>
