  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-black text-surface-dark text-xl">Berichten</h2>
          <p className="text-sm text-warm-gray/60 mt-0.5">Communiceer direct met ons team</p>
        </div>
      ...
      <AnimatePresence>
        {showNew && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            <h3 className="font-bold text-surface-dark">Nieuw gesprek starten</h3>
            ...
            <div className="flex gap-3">
              <button onClick={() => setShowNew(false)} className="text-sm text-warm-gray/70 px-5 py-2.5 hover:text-warm-gray transition-colors font-medium">Annuleren</button>
          ...
          {convs.length === 0 ? (
            <div className="card-premium p-10 text-center">
              <div className="w-14 h-14 bg-gradient-to-br from-primary/10 to-ocean/10 rounded-2xl flex items-center justify-center mx-auto mb-3">
                <MessageSquare size={24} className="text-warm-gray/40" />
              </div>
              <p className="text-sm text-warm-gray/70 font-bold">Nog geen gesprekken</p>
              <p className="text-xs text-warm-gray/50 mt-1">Start een nieuw bericht hierboven</p>
            </div>
          ) : convs.map((c, i) => (
            <motion.button key={c.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
              onClick={() => openConv(c)}
              className={`w-full text-left rounded-xl border p-4 transition-all hover:shadow-md ${selected?.id === c.id ? 'bg-primary/[0.06] border-primary/30 shadow-md shadow-primary/10' : 'bg-surface border-sand-dark/20 hover:border-primary/20'}`}>
              <div className="flex items-center justify-between">
                <span className="text-sm font-bold text-surface-dark truncate">{c.subject}</span>
              ...
              <p className="text-xs text-warm-gray/60 mt-1.5 truncate">{c.last_message?.substring(0, 60)}</p>
              {c.last_message_at && <p className="text-xs text-warm-gray/40 mt-1">{new Date(c.last_message_at).toLocaleDateString('nl-NL', { day: 'numeric', month: 'short' })}</p>}
            </motion.button>
          ))}
        </div>
        ...
        <div className="lg:col-span-2">
          {selected ? (
            <div className="card-premium flex flex-col h-[500px] overflow-hidden">
              <div className="p-4 border-b border-sand-dark/15 flex items-center gap-3 bg-gradient-to-r from-sand/30 to-transparent">
                <button onClick={() => setSelected(null)} className="lg:hidden text-warm-gray/60 hover:text-warm-gray"><ArrowLeft size={18} /></button>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3 bg-sand/20">
                {msgs.map((m, i) => (
                  <motion.div key={m.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
                    ...
                    <div className={`max-w-[80%] rounded-2xl px-4 py-3 shadow-sm ${m.sender_type === 'customer' ? 'bg-gradient-to-br from-primary to-primary-dark text-white rounded-br-md' : 'bg-white text-surface-dark border border-sand-dark/15 rounded-bl-md'}`}>
                      ...
                      <p className={`text-xs mt-1.5 ${m.sender_type === 'customer' ? 'text-white/50' : 'text-warm-gray/50'}`}>
              ...
              <div className="p-4 border-t border-sand-dark/15 flex gap-2 bg-surface">
                <input value={reply} onChange={e => setReply(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendReply(); }}} placeholder="Typ een reactie..." className="flex-1 px-4 py-2.5 border border-sand-dark/30 rounded-xl text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none bg-sand/20 focus:bg-white transition-all" />
            ...
            <div className="card-premium p-14 text-center h-[500px] flex flex-col items-center justify-center">
              <div className="w-16 h-16 bg-gradient-to-br from-primary/10 to-ocean/10 rounded-2xl flex items-center justify-center mb-4">
                <MessageSquare size={28} className="text-warm-gray/30" />
              </div>
              <p className="text-warm-gray/60 font-bold">Selecteer een gesprek</p>
              <p className="text-warm-gray/40 text-sm mt-1">Of start een nieuw bericht</p>
            </div>
          )}
        </div>
      </div>
