    ...
    <div className="max-w-2xl">
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="card-premium p-6 sm:p-8">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h2 className="font-black text-surface-dark text-xl">Vrienden uitnodigen</h2>
            <p className="text-sm text-warm-gray/70">Verdien korting op uw stalling</p>
          </div>
        </div>

        {/* How it works */}
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          className="bg-gradient-to-br from-sand/60 to-sand/30 rounded-2xl p-6 mb-6 border border-sand-dark/15">
          ...
          <div className="space-y-3">
            {[
              ...
              <motion.div key={s.step} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.15 + i * 0.06 }}
                ...
                <p className="text-sm text-warm-gray pt-0.5">{s.text}</p>
              </motion.div>
            ))}
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-6">
          <label className="text-xs font-bold text-warm-gray/60 uppercase tracking-wider block mb-2">Uw doorverwijslink</label>
          <div className="flex items-center gap-2">
            <input
              readOnly
              value={referralUrl}
              className="flex-1 border border-sand-dark/20 rounded-xl px-4 py-3.5 text-sm bg-sand/30 focus:outline-none font-mono text-warm-gray/80"
            />
            <button
              onClick={handleCopy}
