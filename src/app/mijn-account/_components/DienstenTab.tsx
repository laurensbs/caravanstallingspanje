  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="font-bold text-gray-900 text-xl">Dienstaanvragen</h2>
          <p className="text-sm text-gray-500/60 mt-0.5">Reparatie, onderhoud, keuring of transport</p>
        </div>
        {!showForm && (
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
            ...
            <div className="flex items-center justify-between mb-5">
              <h3 className="font-bold text-gray-900 text-lg">Nieuwe aanvraag</h3>
              <button onClick={() => setShowForm(false)} className="text-gray-500/50 hover:text-gray-500 transition-colors"><X size={18} /></button>
            </div>
            <form onSubmit={submitService} className="space-y-5 max-w-lg">
              <div>
                <label className="text-xs font-bold text-gray-500/60 uppercase tracking-wider block mb-2">Caravan *</label>
              ...
              <div>
                <label className="text-xs font-bold text-gray-500/60 uppercase tracking-wider block mb-2">Type dienst *</label>
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                  {Object.entries(TYPE_LABELS).map(([val, label]) => (
                    <button key={val} type="button" onClick={() => setServiceForm({ ...serviceForm, service_type: val })}
                      className={`px-3 py-2.5 rounded-xl text-sm font-semibold border transition-all ${serviceForm.service_type === val ? 'bg-primary/[0.08] border-primary/30 text-primary ring-1 ring-primary/20' : 'bg-gray-50 border-gray-200 text-gray-500/70 hover:border-primary/20'}`}>
                      {label}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="text-xs font-bold text-gray-500/60 uppercase tracking-wider block mb-2">Omschrijving *</label>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowForm(false)} className="px-5 py-3 text-sm text-gray-500/70 hover:text-gray-500 font-medium transition-colors">Annuleren</button>
      ...
      {loadingRequests ? (
        ...
        <div className="space-y-3">
          <h3 className="text-sm font-bold text-gray-500/60 uppercase tracking-wider">Lopende aanvragen ({activeRequests.length})</h3>
              ...
              <motion.div key={r.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
                ...
                <div className="flex items-start justify-between gap-3 mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <span className="font-bold text-gray-900">{TYPE_LABELS[r.service_type] || r.service_type}</span>
                      <span className="text-gray-500/40 ml-2">#{r.id}</span>
                      <p className="text-xs text-gray-500/60 mt-0.5">{r.caravan_name} &middot; {r.license_plate}</p>
                    </div>
                  </div>
                ...
                <div className="mb-3">
                  ...
                  <div className="flex justify-between mt-1.5">
                    {STEP_ORDER.map((step, si) => (
                      <span key={step} className={`text-[10px] font-medium ${si <= stepIdx ? 'text-primary' : 'text-gray-500/30'}`}>
                        {STATUS_CONFIG[step]?.label}
                      </span>
                    ))}
                  </div>
                </div>
                {r.description && <p className="text-sm text-gray-500/60 line-clamp-2 mb-3">{r.description}</p>}
                <div className="flex flex-wrap gap-4 text-xs text-gray-500/50">
      ...
      {!loadingRequests && pastRequests.length > 0 && (
        <div className="space-y-3">
          <h3 className="text-sm font-bold text-gray-500/60 uppercase tracking-wider">Afgerond ({pastRequests.length})</h3>
          {pastRequests.map(r => {
            const sc = STATUS_CONFIG[r.status] || STATUS_CONFIG.afgerond;
            const Icon = sc.icon;
            return (
              <div key={r.id} className="bg-surface rounded-xl border border-gray-200 p-4 opacity-70 hover:opacity-100 transition-opacity">
                <div className="flex items-start justify-between gap-3 mb-1">
                  <span className="font-bold text-gray-900 text-sm">{TYPE_LABELS[r.service_type] || r.service_type} <span className="text-gray-500/40 font-normal">#{r.id}</span></span>
                ...
                <p className="text-sm text-gray-500/60">{r.caravan_name}</p>
                <div className="flex flex-wrap gap-4 mt-2 text-xs text-gray-500/50">
                  <span>Ingediend: {new Date(r.created_at).toLocaleDateString('nl-NL')}</span>
      ...
      {!loadingRequests && requests.length === 0 && (
        <div className="card-premium p-12 text-center">
          <div className="w-16 h-16 bg-gradient-to-br from-primary/10 to-accent/10 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <Wrench size={28} className="text-gray-500/30" />
          </div>
          <p className="text-gray-500/60 font-bold text-lg mb-1">Geen aanvragen</p>
          <p className="text-gray-500/40 text-sm">Dien uw eerste serviceverzoek in via de knop hierboven</p>
        </div>
      )}
    </div>
  );
