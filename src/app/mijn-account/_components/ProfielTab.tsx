...
export default function ProfielTab({ customer }: Props) {
  ...
  const inputClass = 'w-full border border-gray-200 rounded-xl px-4 py-3 text-sm bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary focus:bg-white transition-all placeholder:text-gray-500/40';
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        ...
        <div className="flex items-center gap-4">
          <div className="w-14 h-14 bg-gradient-to-br from-primary to-primary-dark rounded-2xl flex items-center justify-center shadow-lg shadow-primary/20">
            <span className="text-white text-xl font-bold">{customer?.name?.charAt(0) || 'U'}</span>
          </div>
          <div>
            <h2 className="text-xl font-bold text-gray-900">{customer?.name}</h2>
            <div className="flex items-center gap-4 mt-1">
              <span className="text-sm text-gray-500/70 flex items-center gap-1.5"><Mail size={13} /> {customer?.email}</span>
              {customer?.phone && <span className="text-sm text-gray-500/70 flex items-center gap-1.5"><Phone size={13} /> {customer.phone}</span>}
            </div>
          </div>
        </div>
      </motion.div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }}
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div><h2 className="font-bold text-gray-900 text-lg">Profiel bewerken</h2><p className="text-xs text-gray-500/60">Werk uw contactgegevens bij</p></div>
          </div>
          {profileMsg && (
          ...
          <form onSubmit={handleProfileUpdate} className="space-y-4">
            <div>
              <label className="text-xs font-bold text-gray-500/60 uppercase tracking-wider block mb-2">Telefoon</label>
              <div className="relative">
                <Phone size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500/30" />
            ...
            <div>
              <label className="text-xs font-bold text-gray-500/60 uppercase tracking-wider block mb-2">Adres</label>
              <div className="relative">
                <MapPin size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500/30" />
            ...
            <div className="grid grid-cols-2 gap-4">
              <div><label className="text-xs font-bold text-gray-500/60 uppercase tracking-wider block mb-2">Postcode</label><input value={profileForm.postal_code} onChange={e => setProfileForm({ ...profileForm, postal_code: e.target.value })} placeholder="1234 AB" className={inputClass} /></div>
              <div><label className="text-xs font-bold text-gray-500/60 uppercase tracking-wider block mb-2">Plaats</label><input value={profileForm.city} onChange={e => setProfileForm({ ...profileForm, city: e.target.value })} placeholder="Amsterdam" className={inputClass} /></div>
            </div>
            <div>
              <label className="text-xs font-bold text-gray-500/60 uppercase tracking-wider block mb-2">Land</label>
              <div className="relative">
                <Globe size={15} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-500/30" />
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.2 }}
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div><h2 className="font-bold text-gray-900 text-lg">Wachtwoord wijzigen</h2><p className="text-xs text-gray-500/60">Beveilig uw account</p></div>
          </div>
          {pwChangeMsg && (
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
          ...
          <form onSubmit={handlePasswordChange} className="space-y-4">
            <div><label className="text-xs font-bold text-gray-500/60 uppercase tracking-wider block mb-2">Huidig wachtwoord</label><input type="password" required value={pwChangeForm.current} onChange={e => setPwChangeForm({ ...pwChangeForm, current: e.target.value })} className={inputClass} placeholder="••••••••" /></div>
            <div><label className="text-xs font-bold text-gray-500/60 uppercase tracking-wider block mb-2">Nieuw wachtwoord</label><input type="password" required minLength={8} value={pwChangeForm.newPw} onChange={e => setPwChangeForm({ ...pwChangeForm, newPw: e.target.value })} className={inputClass} placeholder="Min. 8 tekens" /></div>
            <div><label className="text-xs font-bold text-gray-500/60 uppercase tracking-wider block mb-2">Herhaal nieuw wachtwoord</label><input type="password" required minLength={8} value={pwChangeForm.confirm} onChange={e => setPwChangeForm({ ...pwChangeForm, confirm: e.target.value })} className={inputClass} placeholder="Bevestig wachtwoord" /></div>
