    ...
    <>
        ...
        <div className="relative w-full max-w-7xl mx-auto px-4 sm:px-6 py-20 sm:py-28">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-center lg:text-left">
              <div className="inline-flex items-center gap-2 bg-white/10 border border-white/15 text-white/90 px-4 py-1.5 rounded-full text-xs font-medium mb-6">
              ...
              {/* Trust */}
              <div className="flex flex-wrap items-center gap-4 sm:gap-6 mt-10 pt-8 border-t border-white/10 justify-center lg:justify-start">
                {[
                  ...
                  <div key={b.text} className="flex items-center gap-2">
                    <b.icon size={14} className="text-white/60" />
            ...
            <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2 }}>
              <div className="booking-widget rounded-2xl p-6 sm:p-8">
                <div className="text-center mb-6">
                  <h2 className="text-lg font-bold text-gray-900">Direct stalling aanvragen</h2>
                  <p className="text-gray-500 text-xs mt-1">Costa Brava, Spanje</p>
                </div>

                {availSpots !== null ? (
                  <div className="text-center py-8">
                    <CheckCircle size={40} className="text-success mx-auto mb-3" />
                    <p className="font-bold text-gray-900">{availSpots} plekken beschikbaar!</p>
                    <p className="text-sm text-gray-500 mt-1">U wordt doorgestuurd naar de reserveringspagina...</p>
                    <div className="mt-4 w-8 h-8 border-2 border-primary border-t-transparent rounded-full animate-spin mx-auto" />
                  </div>
                ) : (
                  <form onSubmit={handleBooking} className="space-y-4">
                    <div>
                      <label className="text-xs font-semibold text-gray-500 block mb-2 uppercase tracking-wider">Type stalling</label>
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                        {[
                          { val: "buiten", label: "Buitenstalling", price: "€65/mnd" },
                          { val: "binnen", label: "Binnenstalling", price: "€95/mnd" },
                        ].map(o => (
                          <button key={o.val} type="button" onClick={() => setBooking({ ...booking, type: o.val })} className={`p-3 rounded-xl text-center transition-all text-xs border ${booking.type === o.val ? "bg-primary/[0.06] border-primary text-primary font-bold ring-1 ring-primary/15" : "bg-gray-50 border-gray-200 text-gray-600 hover:border-primary/20"}`}>
                            <div className="font-semibold text-sm">{o.label}</div>
                            <div className={`text-xs mt-0.5 ${booking.type === o.val ? "text-primary/60" : "text-gray-400"}`}>{o.price}</div>
                          </button>
                        ))}
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-xs font-semibold text-gray-500 block mb-2 uppercase tracking-wider">Lengte caravan</label>
                        <div className="relative">
                          <Ruler size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                          <select value={booking.length} onChange={e => setBooking({ ...booking, length: e.target.value })} required className="w-full pl-9 pr-3 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 focus:ring-2 focus:ring-primary/15 focus:border-primary/30 outline-none transition-all appearance-none">
                      ...
                      <div>
                        <label className="text-xs font-semibold text-gray-500 block mb-2 uppercase tracking-wider">Startdatum</label>
                        <div className="relative">
                          <Calendar size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                          <input type="date" required value={booking.start} onChange={e => setBooking({ ...booking, start: e.target.value })} className="w-full pl-9 pr-3 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 focus:ring-2 focus:ring-primary/15 focus:border-primary/30 outline-none transition-all" />
                        </div>
                      </div>
                    </div>
                    <div>
                      <label className="text-xs font-semibold text-gray-500 block mb-2 uppercase tracking-wider">Locatie</label>
                      <div className="relative">
                        <MapPin size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                        <select value={booking.location} onChange={e => setBooking({ ...booking, location: e.target.value })} className="w-full pl-9 pr-3 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 focus:ring-2 focus:ring-primary/15 focus:border-primary/30 outline-none transition-all appearance-none">
                    ...
                    <button type="submit" disabled={checkingAvail} className="w-full bg-primary hover:bg-primary-light text-white font-semibold py-3.5 rounded-xl text-sm transition-all duration-200 flex items-center justify-center gap-2 shadow-sm hover:shadow-md disabled:opacity-60">
      ...
      <section className="py-10 sm:py-16 bg-surface">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="text-center mb-8">
            <h2 className="text-2xl sm:text-3xl font-bold">Wat kunnen wij voor u doen?</h2>
            <p className="text-gray-500 text-sm mt-2">Tik op een dienst om direct aan te vragen</p>
          </div>
            ...
            {[
              ...
              <A key={s.label} delay={i * 0.05}>
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-gray-900/90 via-gray-900/50 to-gray-900/20 group-hover:from-gray-900/95 group-hover:via-gray-900/60 transition-all duration-300" />
                  <div className="absolute inset-0 flex flex-col items-center justify-center text-white p-3">
                    ...
                    <span className="font-bold text-sm sm:text-base text-center leading-tight">{s.label}</span>
          ...
          <div className="text-center mt-6">
            <Link href="/diensten" className="text-sm text-primary font-semibold hover:text-primary-light transition-colors inline-flex items-center gap-1">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 pb-10 sm:pb-16 w-full">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
              <div>
                ...
                <h3 className="text-white text-xl sm:text-3xl font-bold leading-tight">3 beveiligde locaties<br className="sm:hidden" /> aan de Costa Brava</h3>
              </div>
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 relative">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 sm:gap-8">
            {[
              ...
              >
                <div className="text-3xl sm:text-5xl font-bold text-white mb-1 font-heading tracking-tight">{s.value}</div>
      ...
      <section className="py-14 sm:py-24 bg-card relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto px-4 sm:px-6 relative">
          <A className="text-center max-w-2xl mx-auto mb-10 sm:mb-16">
            ...
            <h2 className="text-2xl sm:text-4xl font-bold mb-3">Hoe werkt het?</h2>
                ...
                <A key={s.step} delay={i * 0.1}>
                  <div className="card-premium p-5 sm:p-6 text-center h-full">
                    <div className="relative inline-flex mb-4 sm:mb-5">
                      ...
                      <span className="absolute -top-1.5 -right-1.5 w-6 h-6 bg-primary text-white text-xs font-bold rounded-full flex items-center justify-center shadow-lg shadow-primary/30">{s.step}</span>
                    </div>
      ...
      <section className="py-12 sm:py-20 bg-surface">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <A className="text-center max-w-2xl mx-auto mb-8 sm:mb-12">
            ...
            <h2 className="text-2xl sm:text-4xl font-bold mb-3">Neem een kijkje op ons terrein</h2>
            <div className="divider-animated mt-3 mb-3" />
          </A>
          ...
          <A>
            <div className="grid lg:grid-cols-2 gap-8 lg:gap-20 items-center">
              <div className="text-center lg:text-left">
                ...
                <h2 className="text-2xl sm:text-4xl font-bold mb-4">Alles online inzien</h2>
                <div className="section-divider mt-0 mb-4 mx-auto lg:mx-0" />
                <p className="text-gray-500 mb-6 leading-relaxed max-w-lg mx-auto lg:mx-0 text-sm sm:text-base">Beheer uw stalling online. Contracten, facturen, inspecties en service aanvragen via uw dashboard.</p>
                ...
                <Link href="/mijn-account" className="inline-flex items-center gap-2 bg-primary hover:bg-gray-800 text-white font-bold px-7 py-3.5 rounded-xl text-sm transition-all duration-200">
      ...
      <section className="py-12 sm:py-20 bg-surface">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <A className="text-center max-w-2xl mx-auto mb-8 sm:mb-14">
            ...
            <h2 className="text-2xl sm:text-4xl font-bold mb-3">Kennis & reisgidsen</h2>
            <div className="section-divider mt-3 mb-3" />
            <p className="text-gray-500 leading-relaxed text-sm">Praktische tips, reisgidsen voor de Costa Brava en alles over caravanonderhoud.</p>
          </A>
          ...
          <div className="text-center mt-10">
            <Link href="/blog" className="inline-flex items-center gap-2 text-sm font-bold text-primary hover:text-primary-light transition-colors">
      ...
      <section className="bg-gray-900 relative overflow-hidden">
        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-12 sm:py-20 text-center relative">
          <A>
            <h2 className="text-2xl sm:text-3xl font-bold text-white mb-4">Klaar om uw caravan veilig te stallen?</h2>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
              <button onClick={() => setQuizOpen(true)} className="bg-primary hover:bg-primary-light text-white font-bold px-8 py-3.5 rounded-xl text-sm transition-all duration-200 inline-flex items-center gap-2 shadow-sm cursor-pointer">
