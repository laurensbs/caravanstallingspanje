...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await logActivity({ actor: admin.name, role: admin.role, action: 'Contract bijgewerkt', entityType: 'contract', entityId: id, entityLabel: `Contract #${id}`, details: data.status ? `Status: ${data.status}` : undefined });
